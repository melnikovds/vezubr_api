...
@allure.feature("CDR")
@allure.description("Обновление Заявки до принятия")
@pytest.mark.parametrize("cargo_count", [50])
def test_update1_cdr_lkz(
    lkz_ext_token,
    lkz_token,
    lkp_token,
    cargo_count
    ...
    with allure.step("ЧАСТЬ 3: Обновление заявки"):
        cdr_response = cdr_client.update_delivery_request(
            cdr_id=cdr_id,
            client_identifier=client_identifier,
    ...
    with allure.step("ЧАСТЬ 4: Проверка статусов грузомест"):
...
@allure.story("Smoke test")
@allure.feature("CDR")
@allure.description("Обновление Заявки после принятия")
@pytest.mark.parametrize("cargo_count", [20])
def test_update2_cdr_lkz(
    lkz_ext_token,
    lkz_token,
    lkp_token,
    cargo_count
    ...
    with allure.step("ЧАСТЬ 2: Создание заявки"):
        ...
        client_identifier = f"CDR-UPDATE-ACTIVE-{datetime.now().strftime('%d%m%Y-%H%M%S')}"
    ...
    with allure.step("ЧАСТЬ 4: Обновление заявки"):
        cdr_response = cdr_client.update_active_delivery_request(
            cdr_id=cdr_id,
            cargo_places=cargo_places_for_cdr,
            route=[
                {"position": 1, "point": 27030, "isLoadingWork": True, "isUnloadingWork": False},
    ...
    with allure.step("ЧАСТЬ 5: Принятие изменений"):
...
@allure.story("Smoke test")
@allure.feature("CDR")
@allure.description("Обновление Заявки после начала исполнения")
@pytest.mark.parametrize("cargo_count", [10])
def test_update3_cdr_lkz(
    lkz_ext_token,
    lkz_token,
    lkp_token,
    cargo_count
    ...
    with allure.step("ЧАСТЬ 7: Обновление заявки"):
        cdr_response = cdr_client.update_active_delivery_request(
            cdr_id=cdr_id,
            cargo_places=cargo_places_for_cdr,
            route=[
    ...
    with allure.step("ЧАСТЬ 8: Принятие изменений"):
    ...
    # with allure.step("ЧАСТЬ 9: Проверка статусов грузомест"):            # здесь тест сейчас падает, ожидает исправления
    #     gm_client_base = CargoPlaceClient(BASE_URL, lkz_token)
    #
    #     # Проверяем случайные 10 грузомест
    #     failed_validations = []
    ...
    with allure.step("ЧАСТЬ 11: Проверка статусов грузомест"):
        gm_client_base = CargoPlaceClient(BASE_URL, lkz_token)
    ...
    with allure.step("ЧАСТЬ 12: Выполнение работ на втором адресе"):
    ...
    with allure.step("ЧАСТЬ 13: Проверка деталки заявки ЛКП"):
        cdr_details = cdr_client_lkp.get_cdr_details(cdr_id)
        assert cdr_details, "Пустой ответ деталки заявки"
    ...
    with allure.step("ЧАСТЬ 14: Проверка деталки заявки ЛКЗ"):
    ...
    with allure.step("ЧАСТЬ 15: Проверка деталки рейса"):
    ...
    with allure.step("ЧАСТЬ 16: Проверка статусов грузомест"):
        gm_client_base = CargoPlaceClient(BASE_URL, lkz_token)
