...
@allure.feature("CDR")
@allure.description("Выполнение Заявки с грузоместами на PROD")
@pytest.mark.parametrize("cargo_count", [500])
def test_cdr_prod_lkz(
    lkz_ext_token_prod,
    lkz_token_prod,
    lkp_token_prod,
    cargo_count
    ...
    with allure.step("ЧАСТЬ 1: Создание грузомест"):
        gm_client_ext = CargoPlaceClient(make_external_url('ru'), lkz_ext_token_prod)

        cargo_list = gm_client_ext.generate_cargo_places_list(
            count=cargo_count,
            role="lkz_ext",
            use_predefined_addresses=False,
            departure_external_id='AUTO 601',
            delivery_external_id='AUTO 602'
        )
    ...
    with allure.step("ЧАСТЬ 2: Создание заявки"):
        cargo_places_for_cdr = [
            {"id": cid, "arrivalPoint": 253963, "departurePoint": 253961}
            for cid in cargo_place_ids
        ]

        cdr_client = CargoDeliveryRequestClient(make_base_url('ru'), lkz_token_prod)
        cdr_response = cdr_client.create_and_publish_delivery_request(
            ...
            point_change_type=2,
            route=[
                {"position": 1, "point": 253961, "isLoadingWork": True, "isUnloadingWork": False},
                {"position": 2, "point": 253963, "isLoadingWork": False, "isUnloadingWork": True}
            ],
    ...
    with allure.step("ЧАСТЬ 3: Принятие обязательств подрядчиком"):
        cdr_client_lkp = CargoDeliveryRequestClient(make_base_url('ru'), lkp_token_prod)
    ...
    with allure.step("ЧАСТЬ 5: Проверка статусов грузомест"):
        gm_client_base = CargoPlaceClient(make_base_url('ru'), lkz_token_prod)
