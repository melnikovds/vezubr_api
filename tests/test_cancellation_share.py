...
@allure.feature("CDR")
@allure.description("Отмена шаринга Заявки в статусе 'Поиск исполнителя'")
@pytest.mark.parametrize("cargo_count", [50])
def test_cancel1_share_lkz(
    lkz_ext_token,
    lkz_token,
    cargo_count
):
    ...
    with allure.step("ЧАСТЬ 2: Создание и публикация заявки"):
    ...
    with allure.step("ЧАСТЬ 3: Проверка деталки заявки ЛКЗ"):
    ...
    with allure.step("ЧАСТЬ 4: Отмена шаринга"):
        cdr_client = CargoDeliveryRequestClient(BASE_URL, lkz_token)

        take_response = cdr_client.cancel_share_cdr(cdr_id)

        # Проверка ответа
        assert take_response is not None, "Отменить шаринг не удалось"
        print(f"✅ Шаринг по Заявке {cdr_id} отменён")
    ...
    with allure.step("ЧАСТЬ 6: Проверка статусов грузомест"):
            ...
            with allure.step(f"Проверка грузоместа {cargo_id}"):
                try:
                    ...
                    assert cdr_id in cdr_ids_in_gm, \
                        f"ГМ {cargo_id}: Заявка {cdr_id} найдена в грузоместе"

                    print(f"ГМ {cargo_id}: cargoDeliveryRequests содержит заявку {cdr_id}")
...
@allure.story("Smoke test")
@allure.feature("CDR")
@allure.description("Отмена шаринга Заявки в статусе 'Подтверждён'")
@pytest.mark.parametrize("cargo_count", [100])
def test_cancel2_share_lkz(
    lkz_ext_token,
    lkz_token,
    lkp_token,
    cargo_count
    ...
    with allure.step("ЧАСТЬ 4: Отмена шаринга"):
        ...
        take_response = cdr_client.cancel_share_cdr(cdr_id, negative=True)

        # Проверка ответа
        assert take_response is not None, "Отменить шаринг удалось"
        print(f"✅ Шаринг по Заявке {cdr_id} не отменён")
...
@allure.story("Smoke test")
@allure.feature("CDR")
@allure.description("Отмена шаринга Заявки в статусе 'В исполнении'")
@pytest.mark.parametrize("cargo_count", [100])
def test_cancel3_share_lkz(
    lkz_ext_token,
    lkz_token,
    lkp_token,
    cargo_count
    ...
    with allure.step("ЧАСТЬ 8: Отмена шаринга"):
        cdr_client = CargoDeliveryRequestClient(BASE_URL, lkz_token)

        take_response = cdr_client.cancel_share_cdr(cdr_id, negative=True)

        # Проверка ответа
        assert take_response is not None, "Удалось отменить шаринг"
        print(f"✅ Шаринг по Заявке {cdr_id} не отменён")

    time.sleep(2)

    with allure.step("ЧАСТЬ 9: Проверка деталки заявки ЛКЗ"):
        cdr_details = cdr_client.get_cdr_details(cdr_id)
    ...
    with allure.step("ЧАСТЬ 10: Выполнение работ на первом адресе"):
    ...
    with allure.step("ЧАСТЬ 11: Отмена шаринга"):
        cdr_client = CargoDeliveryRequestClient(BASE_URL, lkz_token)
    ...
    with allure.step("ЧАСТЬ 12: Проверка деталки заявки ЛКЗ"):
        cdr_details = cdr_client.get_cdr_details(cdr_id)
    ...
    with allure.step("ЧАСТЬ 13: Выполнение работ на втором адресе"):
    ...
    with allure.step("ЧАСТЬ 14: Проверка деталки заявки ЛКП"):
        cdr_details = cdr_client_lkp.get_cdr_details(cdr_id)
        assert cdr_details, "Пустой ответ деталки заявки"
    ...
    with allure.step("ЧАСТЬ 15: Проверка деталки заявки ЛКЗ"):
        cdr_details = cdr_client.get_cdr_details(cdr_id)
    ...
    with allure.step("ЧАСТЬ 17: Проверка статусов грузомест"):
        gm_client_base = CargoPlaceClient(BASE_URL, lkz_token)
...
@allure.story("Smoke test")
@allure.feature("CDR")
@allure.description("Отмена шаринга Заявки в статусе 'Завершён'")
@pytest.mark.parametrize("cargo_count", [100])
def test_cancel4_share_lkz(
    lkz_ext_token,
    lkz_token,
    lkp_token,
    cargo_count
    ...
    with allure.step("ЧАСТЬ 8: Выполнение работ на первом адресе"):
    ...
    with allure.step("ЧАСТЬ 9: Выполнение работ на втором адресе"):
    ...
    with allure.step("ЧАСТЬ 10: Проверка деталки заявки ЛКП"):
        cdr_details = cdr_client_lkp.get_cdr_details(cdr_id)
        assert cdr_details, "Пустой ответ деталки заявки"
    ...
    with allure.step("ЧАСТЬ 14: Проверка статусов грузомест"):
        gm_client_base = CargoPlaceClient(BASE_URL, lkz_token)
