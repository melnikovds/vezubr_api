...
@allure.story("Smoke test")
@allure.feature("CDR")
@allure.description("Отмена Заявки с грузоместами Заказчиком (с Рейсом в исполнении)")
@pytest.mark.parametrize("cargo_count", [50])
def test_cancel3_cdr_with_cargo_places_lkz(
    ...
    with allure.step("ЧАСТЬ 7: Отмена заявки Заказчиком"):
        cdr_client = CargoDeliveryRequestClient(BASE_URL, lkz_token)

        take_response = cdr_client.cancel_cdr_lkz(cdr_id, negative=True)

        # Проверка ответа
        assert take_response is not None, "Удалось отменить Заявку"
        print(f"✅ Заявка {cdr_id} не отменена")
            ...
            with allure.step(f"Проверка грузоместа {cargo_id}"):
                try:
                    ...
                    assert cdr_id in cdr_ids_in_gm, \
                        f"ГМ {cargo_id}: Заявка {cdr_id} не найдена в грузоместе, а должна быть. Найдено: {cdr_ids_in_gm}"
    ...
    with allure.step("ЧАСТЬ 11: Отмена заявки Заказчиком"):
        cdr_client = CargoDeliveryRequestClient(BASE_URL, lkz_token)
    ...
    with allure.step("ЧАСТЬ 14: Выполнение работ на втором адресе"):
...
def test_cancel1_cdr_with_cargo_places_lkp(
    lkz_ext_token,
    lkz_token,
    lkp_token,
    cargo_count
    ...
    with allure.step("ЧАСТЬ 4: Проверка деталки заявки ЛКП"):
        cdr_details = cdr_client_lkp.get_cdr_details(cdr_id)
        assert cdr_details, "Пустой ответ деталки заявки"
    ...
    with allure.step("ЧАСТЬ 5: Отмена заявки Подрядчиком"):
        cdr_client_lkp = CargoDeliveryRequestClient(BASE_URL, lkp_token)

        take_response = cdr_client_lkp.cancel_cdr_lkp(cdr_id)
    ...
    with allure.step("ЧАСТЬ 6: Проверка деталки заявки ЛКЗ"):
        cdr_client_lkz = CargoDeliveryRequestClient(BASE_URL, lkz_token)
        cdr_details = cdr_client_lkz.get_cdr_details(cdr_id)

        expected_status = "canceled_by_producer"
        actual_status = cdr_details.get("status")
        assert actual_status == expected_status, \
            f"Ожидался статус заявки '{expected_status}', получен: '{actual_status}'"
        print(f"✅ Статус заявки: {actual_status}")
    ...
    with allure.step("ЧАСТЬ 7: Проверка деталки заявки ЛКП"):
        cdr_details = cdr_client_lkp.get_cdr_details(cdr_id)
        assert cdr_details, "Пустой ответ деталки заявки"
...
@pytest.mark.parametrize("cargo_count", [50])
def test_cancel2_cdr_with_cargo_places_lkp(
    lkz_ext_token,
    lkz_token,
    lkp_token,
    cargo_count
    ...
    with allure.step("ЧАСТЬ 7: Отмена заявки Подрядчиком"):
        cdr_client_lkp = CargoDeliveryRequestClient(BASE_URL, lkp_token)
    ...
    with allure.step("ЧАСТЬ 8: Проверка деталки заявки ЛКЗ"):
        cdr_client_lkz = CargoDeliveryRequestClient(BASE_URL, lkz_token)
        cdr_details = cdr_client_lkz.get_cdr_details(cdr_id)
    ...
    with allure.step("ЧАСТЬ 9: Проверка деталки заявки ЛКП"):
        cdr_details = cdr_client_lkp.get_cdr_details(cdr_id)
        assert cdr_details, "Пустой ответ деталки заявки"
...
@allure.story("Smoke test")
@allure.feature("CDR")
@allure.description("Отмена Заявки с грузоместами Подрядчиком (с Рейсом в исполнении)")
@pytest.mark.parametrize("cargo_count", [50])
def test_cancel3_cdr_with_cargo_places_lkp(
    lkz_ext_token,
    lkz_token,
    lkp_token,
    cargo_count
    ...
    with allure.step("ЧАСТЬ 7: Отмена заявки Подрядчиком"):
        ...
        take_response = cdr_client_lkp.cancel_cdr_lkp(cdr_id, negative=True)
    ...
    with allure.step("ЧАСТЬ 10: Отмена заявки Подрядчиком"):
        cdr_client_lkp = CargoDeliveryRequestClient(BASE_URL, lkp_token)
    ...
    with allure.step("ЧАСТЬ 11: Проверка деталки заявки ЛКП"):
        cdr_details = cdr_client_lkp.get_cdr_details(cdr_id)
        assert cdr_details, "Пустой ответ деталки заявки"
    ...
    with allure.step("ЧАСТЬ 13: Проверка статусов грузомест"):
        gm_client_base = CargoPlaceClient(BASE_URL, lkz_token)
