...
@allure.feature("CDR")
@allure.description("Выполнение Заявки с грузоместами на DEV")
@pytest.mark.parametrize("cargo_count", [500])
def test_cdr_dev_lkz(
    lkz_ext_token_dev,
    lkz_token_dev,
    lkp_token_dev,
    cargo_count
    ...
    with allure.step("ЧАСТЬ 2: Создание заявки"):
        cargo_places_for_cdr = [
            {"id": cid, "arrivalPoint": 24552, "departurePoint": 25791}
            for cid in cargo_place_ids
        ]

        cdr_client = CargoDeliveryRequestClient(make_base_url('dev'), lkz_token_dev)
        cdr_response = cdr_client.create_and_publish_delivery_request(
            ...
            point_change_type=2,
            route=[
                {"position": 1, "point": 25791, "isLoadingWork": True, "isUnloadingWork": False},
                {"position": 2, "point": 24552, "isLoadingWork": False, "isUnloadingWork": True}
            ],
    ...
    with allure.step("ЧАСТЬ 3: Принятие обязательств подрядчиком"):
        cdr_client_lkp = CargoDeliveryRequestClient(make_base_url('dev'), lkp_token_dev)
    ...
    with allure.step("ЧАСТЬ 5: Проверка статусов грузомест"):
        gm_client_base = CargoPlaceClient(make_base_url('dev'), lkz_token_dev)
